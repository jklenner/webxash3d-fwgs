/*  RoundEnd Blocker (Xash-safe minimal, safe team-set)
 *  Keeps invisible, intangible alive fakeclients to prevent team-elimination round ends.
 *  Adds hard cleanup + de-duplication across mapchanges to avoid slot leaks.
 *
 *  CVARs:
 *    reb_enable 1/0   - enable plugin (default 1)
 *    reb_delay  10.0  - seconds after round/map start to ensure keepers
 *    reb_hide   1/0   - if 1, display keepers as SPECTATOR on scoreboard (engine team stays T/CT)
 *    reb_team   0/1   - 0 = one T + one CT (recommended), 1 = single keeper on CT
 *
 *  Requires modules: amxmodx, cstrike, fakemeta, hamsandwich, fun
 */

#include <amxmodx>
#include <cstrike>
#include <fakemeta>
#include <hamsandwich>
#include <fun>

#define PLUGIN  "RoundEnd Blocker (Xash-safe)"
#define VERSION "0.9.5"
#define AUTHOR  "jklenner (+ fixes)"

#define NAME_T   "[OS] RoundKeeperT"
#define NAME_CT  "[OS] RoundKeeperCT"

// ----- task id ranges (distinct spaces so removals don't collide)
const TASK_TEAM   = 10000;
const TASK_SPAWN  = 20000;
const TASK_HEALID = 12345;

// ----- state -----
new bool:g_isKeeper[33];           // marks our own fakeclients
new CsTeams:g_targetTeam[33];      // desired team for each keeper
new CsTeams:g_lastTeam[33];        // last team actually applied (prevents noisy resets)
new g_msgTeamInfo;

new bool:g_changingLevel = false;  // set during changelevel / deactivate

// CVARs
new pEnable, pDelay, pHide, pTeam;

// ----- helpers -----
stock bool:is_valid_ent(id)    { return (1 <= id <= get_maxplayers()); }
stock bool:is_in_game_safe(id) { return is_valid_ent(id) && is_user_connected(id); }

stock shutdown_keep_tasks(const bool:wipeFlags)
{
    // stop periodic keepalive task
    remove_task(TASK_HEALID);

    // stop any pending team/spawn tasks
    new maxp = get_maxplayers();
    for (new i = 1; i <= maxp; i++)
    {
        remove_task(TASK_TEAM + i);
        remove_task(TASK_SPAWN + i);

        if (wipeFlags)
        {
            g_isKeeper[i]   = false;
            g_targetTeam[i] = CS_TEAM_UNASSIGNED;
            g_lastTeam[i]   = CS_TEAM_UNASSIGNED;
        }
    }
}

// ===== plugin lifecycle =====

public plugin_init()
{
    register_plugin(PLUGIN, VERSION, AUTHOR);

    pEnable = register_cvar("reb_enable", "1");
    pDelay  = register_cvar("reb_delay",  "10.0");
    pHide   = register_cvar("reb_hide",   "1");  // ON by default (mask keepers as Spectator)
    pTeam   = register_cvar("reb_team",   "0");  // 0=two keepers (T+CT), 1=single keeper on CT

    // round start (HLTV new round frame)
    register_event("HLTV", "OnRoundStart", "a", "1=0", "2=0");

    g_msgTeamInfo = get_user_msgid("TeamInfo");
    register_message(g_msgTeamInfo, "Msg_TeamInfo");

    // Map-change / teardown guards
    register_forward(FM_ChangeLevel,       "fw_ChangeLevel",       1);
    register_forward(FM_ServerDeactivate,  "fw_ServerDeactivate",  1);

    hook_cvar_change(pEnable, "OnRebEnableChanged");
    register_srvcmd("reb_cleanup", "SrvCmd_RebCleanup");

}

public plugin_cfg()
{
    if (get_pcvar_num(pEnable))
    {
        // remove dupes before starting a new round cycle (stale keepers from previous map)
        prune_duplicate_keepers();

        set_task(get_pcvar_float(pDelay), "Task_EnsureKeepers");
        set_task(5.0, "Task_KeepAlive", TASK_HEALID, "", 0, "b");
    }
}

public OnRoundStart()
{
    if (g_changingLevel) return;

    if (get_pcvar_num(pEnable))
    {
        // guard against late survivors / racey init on some paths
        prune_duplicate_keepers();
        set_task(get_pcvar_float(pDelay), "Task_EnsureKeepers");
    }
}

public fw_ChangeLevel(const map[])
{
    g_changingLevel = true;
    cleanup_keepers();           // proactively free slots
    shutdown_keep_tasks(true);   // fully clear tasks & flags
}

public fw_ServerDeactivate()
{
    g_changingLevel = true;
    cleanup_keepers();          // belt & suspenders if ChangeLevel didn't fire
    shutdown_keep_tasks(true);
}

public plugin_end()
{
    // Final cleanup; avoid leaving fakeclients behind
    cleanup_keepers();
}

// AMXX still calls this on Xash; we also provide client_disconnected below.
public client_disconnect(id)
{
    if (g_isKeeper[id])
    {
        g_isKeeper[id]   = false;
        g_targetTeam[id] = CS_TEAM_UNASSIGNED;
        g_lastTeam[id]   = CS_TEAM_UNASSIGNED;

        if (!g_changingLevel && get_pcvar_num(pEnable))
            set_task(get_pcvar_float(pDelay), "Task_EnsureKeepers");
    }
}

// To appease the deprecation warning. On Xash it may never fire; harmless if it does.
public client_disconnected(id)
{
    client_disconnect(id);
}

// When a human/bot joins, force them to see keepers masked as Spectator
public client_putinserver(receiver)
{
    if (g_changingLevel) return;
    if (!get_pcvar_num(pHide)) return;

    new id = find_keeper_by_name(NAME_T);
    if (id) mask_keeper_to_spectator_one(receiver, id);

    id = find_keeper_by_name(NAME_CT);
    if (id) mask_keeper_to_spectator_one(receiver, id);
}

// ===== scoreboard masking =====

public Msg_TeamInfo(msgid, dest, id)
{
    if (g_changingLevel) return PLUGIN_CONTINUE;
    if (!get_pcvar_num(pEnable) || !get_pcvar_num(pHide))
        return PLUGIN_CONTINUE;

    new pid = get_msg_arg_int(1);
    if (1 <= pid <= get_maxplayers() && g_isKeeper[pid])
    {
        // Only visual: show as Spectator on the scoreboard; engine team remains T/CT
        set_msg_arg_string(2, "SPECTATOR");
    }
    return PLUGIN_CONTINUE;
}

// ===== main logic =====

public Task_EnsureKeepers()
{
    if (g_changingLevel || !get_pcvar_num(pEnable)) return;

    if (get_pcvar_num(pTeam) == 0)
    {
        ensure_keeper_for(CS_TEAM_T,  NAME_T);
        ensure_keeper_for(CS_TEAM_CT, NAME_CT);
    }
    else
    {
        ensure_keeper_for(CS_TEAM_CT, NAME_CT); // single-keeper mode
    }
}

public Task_KeepAlive()
{
    if (g_changingLevel || !get_pcvar_num(pEnable)) return;

    if (get_pcvar_num(pTeam) == 0)
    {
        ensure_keeper_for(CS_TEAM_T,  NAME_T);
        ensure_keeper_for(CS_TEAM_CT, NAME_CT);
    }
    else
    {
        ensure_keeper_for(CS_TEAM_CT, NAME_CT);
    }
}

stock ensure_keeper_for(CsTeams:team, const name[])
{
    if (g_changingLevel) return;

    new id = find_keeper_by_name(name);

    if (id && is_user_connected(id))
    {
        // Put/keep on desired team safely (deferred) — ONLY if changed
        if (g_lastTeam[id] != team) schedule_set_team(id, team);

        // Ensure alive & properties
        if (!is_user_alive(id))
        {
            remove_task(TASK_SPAWN + id);
            set_task(0.40, "Task_SpawnKeeper", TASK_SPAWN + id);
        }
        else
        {
            apply_keeper_props(id);
            if (get_pcvar_num(pHide)) mask_keeper_to_spectator_global(id);
        }
        return;
    }

    // Create new fakeclient
    id = create_keeper(name);
    if (!id) return;

    g_isKeeper[id] = true;
    g_lastTeam[id] = CS_TEAM_UNASSIGNED; // ensure first set happens
    schedule_set_team(id, team);         // will set g_targetTeam and defer team set

    // Spawn a tad later (team likely applied by then)
    remove_task(TASK_SPAWN + id);
    set_task(0.45, "Task_SpawnKeeper", TASK_SPAWN + id);
}

stock schedule_set_team(id, CsTeams:team)
{
    if (g_changingLevel) return;
    g_targetTeam[id] = team;
    remove_task(TASK_TEAM + id);
    set_task(0.25, "Task_SetTeam", TASK_TEAM + id);
}

public Task_SetTeam(taskid)
{
    if (g_changingLevel) return;

    new id = taskid - TASK_TEAM;
    if (!(1 <= id <= get_maxplayers())) return;
    if (!g_isKeeper[id]) { remove_task(taskid); return; }

    if (!is_in_game_safe(id))
    {
        // Try again shortly until in-game
        set_task(0.25, "Task_SetTeam", taskid);
        return;
    }

    new CsTeams:team = g_targetTeam[id];
    if (team == CS_TEAM_UNASSIGNED) team = CS_TEAM_CT;

    // Only set if it actually changed (avoids constant model-update path on Xash)
    if (g_lastTeam[id] != team)
    {
        cs_set_user_team(id, team);
        g_lastTeam[id] = team;

        if (get_pcvar_num(pHide)) mask_keeper_to_spectator_global(id);
    }
}

public Task_SpawnKeeper(taskid)
{
    if (g_changingLevel) return;

    new id = taskid - TASK_SPAWN;
    if (!(1 <= id <= get_maxplayers())) return;
    if (!g_isKeeper[id] || !is_user_connected(id)) return;

    if (!is_user_alive(id))
        ExecuteHamB(Ham_Spawn, id);

    apply_keeper_props(id);

    if (get_pcvar_num(pHide)) mask_keeper_to_spectator_global(id);
}

// ===== low-level ops =====

stock create_keeper(const name[])
{
    if (g_changingLevel) return 0;

    // Create fake client, connect, put in server
    new id = engfunc(EngFunc_CreateFakeClient, name);
    if (!id) return 0;

    dllfunc(DLLFunc_ClientConnect, id, name, "127.0.0.1", "127.0.0.1");
    dllfunc(DLLFunc_ClientPutInServer, id);

    // safety flag
    set_pev(id, pev_flags, pev(id, pev_flags) | FL_FAKECLIENT);
    return id;
}

stock apply_keeper_props(id)
{
    if (g_changingLevel) return;
    if (!is_in_game_safe(id)) return;

    // Invisible, intangible, unkillable, non-interactive
    set_pev(id, pev_effects, pev(id, pev_effects) | EF_NODRAW);
    set_pev(id, pev_takedamage, DAMAGE_NO);
    set_pev(id, pev_solid, SOLID_NOT);
    set_pev(id, pev_movetype, MOVETYPE_NOCLIP);
    set_pev(id, pev_flags, pev(id, pev_flags) | FL_NOTARGET);

    // Huge health just in case some plugin toggles takedamage
    set_pev(id, pev_health, 1000000.0);

    // Strip weapons to avoid odd forwards
    strip_user_weapons(id);
}

stock find_keeper_by_name(const name[])
{
    new maxp = get_maxplayers();
    new pname[32];
    for (new i = 1; i <= maxp; i++)
    {
        if (!is_user_connected(i)) continue;
        if (!g_isKeeper[i]) continue;

        get_user_name(i, pname, charsmax(pname));
        if (equal(pname, name)) return i;
    }
    return 0;
}

// ===== TeamInfo masking helpers =====

stock mask_keeper_to_spectator_global(id)
{
    if (g_changingLevel) return;
    if (!is_in_game_safe(id)) return;

    message_begin(MSG_ALL, g_msgTeamInfo);
    write_byte(id);
    write_string("SPECTATOR");
    message_end();
}

stock mask_keeper_to_spectator_one(receiver, id)
{
    if (g_changingLevel) return;
    if (!is_in_game_safe(id) || !is_in_game_safe(receiver)) return;

    message_begin(MSG_ONE, g_msgTeamInfo, _, receiver);
    write_byte(id);
    write_string("SPECTATOR");
    message_end();
}

// ===== hard cleanup & de-dup to prevent slot leaks =====

stock bool:is_keeper_name(id)
{
    new n[32];
    get_user_name(id, n, charsmax(n));
    return equal(n, NAME_T) || equal(n, NAME_CT);
}

stock kick_if_connected(id)
{
    if (!is_user_connected(id)) return;

    // Preferred: kick via userid (if present), it’s cleaner in logs
    new uid = get_user_userid(id);
    if (uid > 0)
    {
        server_cmd("kick #%d ^"reb_cleanup^"", uid);
        server_exec();
    }
    else
    {
        // Last-resort: force through the game DLL
        dllfunc(DLLFunc_ClientDisconnect, id);
    }
}

stock cleanup_keepers()
{
    new maxp = get_maxplayers();
    for (new i = 1; i <= maxp; i++)
    {
        if (!is_user_connected(i)) continue;

        if (g_isKeeper[i] || is_keeper_name(i))
            kick_if_connected(i);

        g_isKeeper[i]   = false;
        g_targetTeam[i] = CS_TEAM_UNASSIGNED;
        g_lastTeam[i]   = CS_TEAM_UNASSIGNED;
    }
}

stock prune_duplicate_keepers()
{
    new maxp = get_maxplayers();
    new firstT = 0, firstCT = 0, n[32];

    for (new i = 1; i <= maxp; i++)
    {
        if (!is_user_connected(i)) continue;

        get_user_name(i, n, charsmax(n));
        if (equal(n, NAME_T))
        {
            if (!firstT) { firstT = i; g_isKeeper[i] = true; }
            else kick_if_connected(i);
        }
        else if (equal(n, NAME_CT))
        {
            if (!firstCT) { firstCT = i; g_isKeeper[i] = true; }
            else kick_if_connected(i);
        }
    }
}

public OnRebEnableChanged(pcvar, const oldValue[], const newValue[])
{
    // If someone sets: reb_enable 0  -> nuke keepers immediately
    if (str_to_num(newValue) <= 0)
    {
        shutdown_keep_tasks(true);  // stop timers & clear flags
        cleanup_keepers();          // kick our fakeclients now
    }
    else
    {
        // turned back on: (re)spawn after the configured delay
        set_task(get_pcvar_float(pDelay), "Task_EnsureKeepers");
    }
}

// Service command you can call via server_cmd("reb_cleanup")
public SrvCmd_RebCleanup()
{
    shutdown_keep_tasks(true);
    cleanup_keepers();
}

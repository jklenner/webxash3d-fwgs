...
{
    // Only touch spawns if Retakes mode is enabled
    if (!get_cvar_num("retakes_mode"))
        return;
...
public plugin_cfg( ) 
{ 
    // Only configure cvars & warmup if Retakes mode is active
    if (!get_cvar_num("retakes_mode"))
        return;
...
{  
    // When not in Retakes mode, don't touch spawns at all
    if (!get_cvar_num("retakes_mode"))
